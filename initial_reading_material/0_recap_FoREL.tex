...
This section will consist of a brief recap of the Follow-the-Regularised-Leader (FoReL) back in Week 7. FoRel \citep{shalev2007online, shalev2007primal} is a simple modification of the Follow-The-Leader (FTL) \citep{kalai2005efficient} learning rule.  In FTL, in each iteration, we select the decision vector (or the next iterate) which minimises the loss on all past iterations
\begin{define}[Follow-The-Leader]
    ...
    \begin{equation}
        x^{(t)} = \arg \min_{x \in S} \left\{\sum_{i = 1}^{t-1} f^{(i)}(x)\right\}\hspace{1cm}\forall t
    \end{equation}
\end{define}
In FoRel, we add a regularisation term $h$. The goal of the regularisation term is to stabilise the solution. \joseph{Maybe consider adding Example 2.2 Failure of FTL to motivate the need of FoREL.}
\begin{define}[Follow-The-Regularised-Leader]\label{defn:forel} The Follow-The-Regularised-Leader (FoREL) learning rule extends FTL by adding a regularisation term $h: S \to \mathbb R$.
    \begin{equation}
        x^{(t)} = \arg \min_{x \in S} \left\{\sum_{i = 1}^{t-1} f^{(i)}(x) + h(x)\right\}\hspace{1cm}\forall t
    \end{equation}
\end{define}
...
\begin{equation}\label{eqn:regret_forel}
    \text{Regret}_T(x) := \sum_{t=1}^T f^{(t)}(x^{(t)}) - \sum_{t=1}^T f^{(t)}(x)
\end{equation}
...
\begin{lemma}\label{lemma:iterates_forel}
    Let $x_1, x_2, \ldots$ be the iterates due to FoReL using the $\sigma$-strongly convex regulariser $h$. Then for all $x \in S$, we have that \begin{equation}\label{eqn:lemma_forel}
        \sum_{t=1}^T (f^{(t)}(x^{(t)}) - f^{(t)}(x)) \leq h(x) - h(x^{(1)}) + \sum_{t=1}^T (f^{(t)}(x^{(t)}) - f^{(t)}(x^{(t+1)})
    \end{equation}
\end{lemma}
Furthermore, it is also shown that if for all $t$, if $f^{(t)}$ is $L_t$-Lipschitz with respect to $||\cdot||$, then we further have that 
