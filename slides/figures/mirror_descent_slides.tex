...
\coordinate (xt)   at (9.9,2.7);   % x^{(t)}
\coordinate (xtp)  at (8.55,0.75); % x^{(t+1)}   (on boundary of X)
\coordinate (xppt) at (7.75,0.45); % \hat{x}^{(t+1)}  (outside X)
...
% ---------- point labels ----------
\node[above left]  at (tt)   {$\theta^{(t)}$};
\node[left]        at (ttp)  {$\theta^{(t+1)}$};
\node[above right] at (xt)   {$x^{(t)}$};
\node[right]       at (xtp)  {$x^{(t+1)}$};
\node[below right] at (xppt) {$\hat{x}^{(t+1)}$};
